...
static inline u8 *esp_tmp_iv(struct crypto_aead *aead, void *tmp, int seqhilen)
{
	return crypto_aead_ivsize(aead) ?
	       PTR_ALIGN((u8 *)tmp + sizeof(struct esp_info) + seqhilen,
			 crypto_aead_alignmask(aead) + 1) : tmp + sizeof(struct esp_info) + seqhilen;
}
...
static int esp6_output_list(struct xfrm_state *x, struct list_head *head)
{
	int alen;
	int blksize;
	struct ip_esp_hdr *esph;
	...
	list_for_each_entry_safe(skb, nskb, head, list) {
		...
		if (slowpath) {
			...
			XFRM_BULK_SKB_CB(skb)->err = esp6_output(x, skb);
			continue;
		}
		...
		*skb_mac_header(skb) = IPPROTO_ESP;
		...
		esp->nfrags = esp6_output_head(x, skb, esp);
		if (esp->nfrags < 0) {
			XFRM_BULK_SKB_CB(skb)->err = esp->nfrags;
			continue;
		}
...
static int esp6_input_done_direct(struct sk_buff *skb, struct xfrm_state *x)
{
	struct crypto_aead *aead = x->data;
...
int esp6_input_done2(struct sk_buff *skb, int err)
{
	...
	return esp6_input_done_direct(skb, x);
...
static void esp_input_set_header(struct sk_buff *skb, struct xfrm_state *x,
				__be32 *seqhi)
{
	/* For ESN we move the header forward by 4 bytes to
	 * accommodate the high bits.  We will move it back after
...
static int esp6_input_list(struct xfrm_state *x, struct list_head *head)
{
	struct crypto_aead *aead = x->data;
...
skip_cow:
		...
		err = esp6_input_done_direct(skb, x);
		XFRM_BULK_SKB_CB(skb)->err = err;
	}
...
static const struct xfrm_type esp6_type = {
	...
	.input		= esp6_input,
	.input_list	= esp6_input_list,
	.output		= esp6_output,
	.output_list	= esp6_output_list,
};
